...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # Learnable lambda parameter for value residual (replaces fixed 0.5)
    ...
    def forward(self, x, v1=None):
        ...
        # If no residual value is provided, use the computed value
        if v1 is None:
            v1 = value
        else:
            v1 = v1.view_as(value)
        # Learnable weighted combination of the two value streams
        ...
        # Return both the attention output and the (combined) value for residual propagation.
...
# New Block implementing Embed Shortcut and Value Residual
class Block(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Instead of the standard block, we use a learnable embed shortcut.
        self.lambdas = nn.Parameter(torch.tensor([1.0, 0.0]))
        self.norm1 = RMSNorm(config.n_embed)
        # Use the modified attention layer (with learnable lambda for value residual)
        self.attention = CausalSelfAttention(config)
        self.norm2 = RMSNorm(config.n_embed)
        # For the second branch, we use the Sparse MoE module.
        self.moe = SparseMoE(config)
    def forward(self, x, x0, v1=None):
        # Embed shortcut: combine current state with the original embedding.
        x = self.lambdas[0] * x + self.lambdas[1] * x0
        # Apply attention with RMSNorm. The first block receives v1 as None.
        attn_out, new_v1 = self.attention(self.norm1(x), v1)
        x = x + attn_out
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            # Create a list of Blocks (with embed shortcut) instead of the original blocks.
            h = nn.ModuleList([Block(config) for _ in range(config.n_layer)]),
            norm_f = RMSNorm(config.n_embed),
        ))
        # Untied lm_head weights
        self.lm_head = nn.Linear(config.n_embed, config.vocab_size, bias=False)
        # Added RMSNorm after embeddings
        self.embed_norm = RMSNorm(config.n_embed)
        # Init lm_head weights to zero as requested
        self.lm_head.weight.data.zero_()

    def forward(self, idx, targets=None):
        B, T = idx.size()
        assert T <= self.config.block_size, f"Sequence length {T} exceeds block size {self.config.block_size}"
        # Get token embeddings and apply embed normalization.
        tok_emb = self.transformer.wte(idx)
        x = self.embed_norm(tok_emb)
        # Save original embedding for the embed shortcut.
        x0 = x
        v1 = None
        # Pass through transformer blocks.
        for i, block in enumerate(self.transformer.h):
            if i == 0:
                x, v1 = block(x, x0, None)
            else:
                x, _ = block(x, x0, v1)
        x = self.transformer.norm_f(x)
        logits = self.lm_head(x)
        # Tanh soft capping of logits.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GPT MoE Training with Rotary, RMSNorm, LR Warmup, Embed Shortcut, and Muon")
