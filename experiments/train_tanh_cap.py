...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            # Use the new Block design instead of the MoE-based one.
    ...
    def forward(self, idx, targets=None):
        ...
        x = tok_emb + pos_emb
        # Apply additional RMSNorm after embedding
        x = self.embed_norm(x)
        # Save a copy for the embed shortcut
        x0 = x
        v1 = None  # For the value residual: first block gets None
        for block in self.transformer.h:
            x, v_new = block(x, x0, v1)
            # For subsequent blocks, use the value produced by the first block.
            if v1 is None:
                v1 = v_new
...
if __name__ == "__main__":
    ...
    muon_params = []
    adam_normal = []
    adam_special = []  # For scalar parameters (e.g., lamb in attention, lambdas in block) with lr=0.02
    ...
    optimizer_adam = torch.optim.AdamW([
        ...
        {'params': adam_special, 'lr': 0.02, 'betas': (0.9, 0.95)}
    ])
    ...
    with mlflow.start_run():
        ...
        for i in tqdm(range(max_iters), desc="Training"):
            ...
            for pg in optimizer_muon.param_groups:
                pg['lr'] = lr_now * 0.1
            # For adam_normal group update main lr; adam_special remains at 0.02.
