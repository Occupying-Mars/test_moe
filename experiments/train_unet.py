...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(
        self,
        params,
        lr=6e-4 * 0.1,  # Note: LR is doubled compared to before.
        momentum=0.95,
        nesterov=True,
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # Create a single linear layer for q, k, v
    ...
    def forward(self, x, v1=None):
        ...
        # Apply value residual mixing: if v1 is not provided, use value itself.
        if v1 is None:
            v1 = value
        else:
            # Ensure v1 has same shape as value
            v1 = v1.view_as(value)
        ...
        # Return both the attention output and the computed value residual for reuse
...
# New transformer block with embed shortcut.
class Block(nn.Module):
    ...
    def forward(self, x, x0, v1=None):
        # Embed shortcut: mix current state with the initial embedding
        x = self.lambdas[0] * x + self.lambdas[1] * x0
        # Attention with RMSNorm; pass along v1 (None for first block)
        attn_in = rms_norm(x, dim=-1)
        attn_out, v_new = self.attn(attn_in, v1)
        x = x + attn_out
        # MLP branch with RMSNorm
        x = x + self.mlp(rms_norm(x, dim=-1))
        return x, v_new

# --------------------------------------------------------------------------
# A small helper linear layer that can cast output if needed.
# For our purposes, it behaves like nn.Linear.
# --------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# NEW GPT BLOCK WITH U-NET STYLE SKIP CONNECTIONS
# -----------------------------------------------------------------------------
class GPT(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            # Instead of a positional embedding, we rely on the blocks (and any internal rotary embeddings).
        ...
        # Untied lm_head weights using our CastedLinear.
        self.lm_head = CastedLinear(config.n_embed, config.vocab_size, bias=False)
        # Initialize lm_head weights to zero.
        self.lm_head.weight.data.zero_()
        # Initialize weights for the rest of the network.
    ...
    def forward(self, idx, targets):
        ...
        # Apply RMSNorm on embeddings.
        x = rms_norm(x, dim=-1)
        x0 = x  # Save a copy for the embed shortcut.
        v1 = None
        ...
        for i in range(self.decoder_layers):
            skip_connection = skip_connections.pop()  # Get corresponding encoder output.
...
# -----------------------------------------------------------------------------
# PRINT DATA STATS & LR SCHEDULING HELPER
# -----------------------------------------------------------------------------
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GPT MoE Training with Rotary, RMSNorm, LR Warmup, and Validation + Muon (with U-Net style GPT block)")
    ...
    # Instantiate the new GPT model with U-Net style skip connections.
    model = GPT(config())
    model.to(device)
    ...
    muon_params = []
    adam_normal = []
    adam_special = []  # For scalar parameters (e.g., lamb in attention, lambdas in block) with lr=0.04 now.
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        # Muon: 2D weight parameters not in embeddings or final head.
    ...
    optimizer_adam = torch.optim.AdamW([
        ...
        {'params': adam_special, 'lr': 0.04, 'betas': (0.9, 0.95)}  # doubled from 0.02
    ])
    ...
    with mlflow.start_run():
        ...
        # If profiling is enabled, wrap training loop in torch.profiler.profile
        if args.profile:
        ...
        for i in tqdm(range(max_iters), desc="Training"):
            ...
            for pg in optimizer_muon.param_groups:
                pg['lr'] = lr_now * 0.1
            # For adam_normal group update main lr; adam_special remains at 0.04.
