...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, v1=None, attn_window=None):
        ...
        if attn_window is not None:
            # Build a mask that allows each query to attend only to the previous attn_window tokens.
...
class Block(nn.Module):
    ...
    def forward(self, x, x0, v1=None, attn_window=None):
        ...
        # Pass attn_window to the attention module.
        attn_out, v_new = self.attn(attn_in, v1, attn_window=attn_window)
        x = x + attn_out
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets, attn_window=None):
        ...
        for i in range(self.encoder_layers):
            x, v1 = self.transformer.h[i](x, x0, v1, attn_window=attn_window)
            skip_connections.append(x)
        # Decoder pass: apply U-net skip connections.
        for i in range(self.decoder_layers):
            skip = skip_connections.pop()
            weighted_skip = self.skip_weights[i] * skip
            x, v1 = self.transformer.h[self.encoder_layers + i](x + weighted_skip, x0, v1, attn_window=attn_window)
