...
class Muon(torch.optim.Optimizer):
    ...
    This optimizer treats each 2D parameter as a matrix and orthogonalizes the
...
class Rotary(nn.Module):
    ...
    def forward(self, x):
        ...
        # head_dim is expected to be equal to 2*(dim//4)
        cos = self.cos[:T].unsqueeze(0).unsqueeze(0)  # shape: (1, 1, T, head_dim//2)
        sin = self.sin[:T].unsqueeze(0).unsqueeze(0)
        return cos, sin

def apply_rotary_emb(x, cos, sin):
    # Assumes x's last dimension splits into two equal parts.
    d = x.shape[-1] // 2
    x1 = x[..., :d]
    x2 = x[..., d:]
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # Pre-computed causal mask for full attention.
    ...
    def forward(self, x, v1=None, attn_window=None):
        ...
        # If an attention window is provided, create a standard causal mask.
...
# -----------------------------------------------------------------------------
# BLOCK MODULE WITH DROPPED ATTENTION LAYER
# -----------------------------------------------------------------------------
class Block(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        # Drop attention layer for block index 7.
        if layer_idx != 7:
...
# -----------------------------------------------------------------------------
# U-NET STYLE SPARSE VALUE EMBEDDING MODULE
# -----------------------------------------------------------------------------
class ValueEmbedding(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        # Create a small ModuleList (using three embeddings) for the encoder.
        self.embeddings = nn.ModuleList([
            nn.Embedding(config.vocab_size, config.n_embed)
            for _ in range(3)
        ])
    def forward(self, inputs):
        # Compute embeddings for the three selected positions.
        v0 = self.embeddings[0](inputs)
        v1 = self.embeddings[1](inputs)
        v2 = self.embeddings[2](inputs)
        # Build a list for all 12 layers: first three, six Nones, and last three (reusing the computed ones)
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # Pass layer indices to blocks so that block 7 drops its attention layer.
    ...
    def forward(self, idx, targets, attn_window=None):
        ...
        x = self.embed_norm(x)
        x0 = x
        # Compute sparse U-net style value embeddings.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GPT MoE Training with U-Net Skip Connections, Rotary, RMSNorm, LR Warmup, Muon, and Sparse U-Net Value Embeddings"
    )
