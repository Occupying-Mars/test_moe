...
# -----------------------------------------------------------------------------
# QK normalization helper function.
def qk_norm(x, eps=1e-6):
    # Normalizes the last dimension by its root-mean-square.
    return x / torch.sqrt(torch.mean(x.pow(2), dim=-1, keepdim=True) + eps)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # Zero-initialize the projection weights
    ...
    def forward(self, x):
        ...
        # Apply QK normalization on query and key
        query = qk_norm(query)
        key   = qk_norm(key)
