...
n_embed = 384
n_head = 6
n_layer = 12  # Updated to 12 layers for value embeddings.
dropout = 0.2
num_experts = 8
top_k = 2
capacity_factor = 1.0
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, v1=None, attn_window=None):
        ...
        # Handle value embedding input
        if v1 is not None:
            # Fix shape mismatch: [1, B, T, C] -> [B, T, C] -> [B, T, n_head, head_dim] -> [B, n_head, T, head_dim]
            v1 = v1.squeeze(0)  # Remove batch dimension
            v1 = v1.view(B, T, self.n_head, self.head_dim).transpose(1, 2)
        else:
            v1 = value

        # Mix values using learned ratio
        ...
        # Attention logic (keep flash attention implementation unchanged)
...
class Block(nn.Module):
    ...
    def forward(self, x, x0, v1=None, attn_window=None):
        ...
        # Pass the dedicated value embedding chunk to the attention module.
...
# -----------------------------------------------------------------------------
# GPT MODEL WITH U-NET STYLE SKIP CONNECTIONS AND VALUE EMBEDDING (vte)
# -----------------------------------------------------------------------------
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            # NEW: Value token embeddings.
            vte = nn.Embedding(config.vocab_size, config.n_embed * config.n_layer),
    ...
    # Updated forward: accepts attn_window and passes it to each block.
    def forward(self, idx, targets, attn_window=None):
        ...
        x = self.embed_norm(x)
        x0 = x
        # NEW: Compute value embeddings and split them into 'n_layer' chunks.
        v_chunks = self.transformer.vte(idx[None]).chunk(self.config.n_layer, dim=-1)
        skip_connections = []
        # Encoder pass: store skip connections.
        for i in range(self.encoder_layers):
            x, _ = self.transformer.h[i](x, x0, v1=v_chunks[i], attn_window=attn_window)
            skip_connections.append(x)
        # Decoder pass: apply U-net skip connections.
        for i in range(self.decoder_layers):
            skip = skip_connections.pop()
            weighted_skip = self.skip_weights[i] * skip
            x, _ = self.transformer.h[self.encoder_layers + i](x + weighted_skip, x0, v1=v_chunks[self.encoder_layers + i], attn_window=attn_window)
    ...
    def generate(self, idx, max_new_tokens, attn_window=None):
        self.eval()
        # For generation, compute v_chunks once per call.
        for _ in range(max_new_tokens):
            idx_cond = idx[:, -self.config.block_size:]
            x = self.transformer.wte(idx_cond)
            x = self.embed_norm(x)
            x0 = x
            # Compute value embeddings for the current context.
            v_chunks = self.transformer.vte(idx_cond[None]).chunk(self.config.n_layer, dim=-1)
            skip_connections = []
            for i in range(self.encoder_layers):
                x, _ = self.transformer.h[i](x, x0, v1=v_chunks[i], attn_window=attn_window)
                skip_connections.append(x)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GPT MoE Training with U-Net Skip Connections, Rotary, RMSNorm, LR Warmup, Muon, and Value Embeddings")
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if param.ndim == 2 and ("wte." not in name) and ("lm_head." not in name) and ("vte" not in name):
    ...
    with mlflow.start_run():
        ...
        for i in tqdm(range(max_iters), desc="Training"):
            ...
            # Compute the warmup attention window (from 64 to block_size)
            ...
            with torch.amp.autocast(device_type=device, dtype=torch.float16, enabled=(device=="cuda")):
                # Pass attn_window to the model forward pass
