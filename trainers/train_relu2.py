...
import argparse
import torch.utils.checkpoint as cp  # For gradient checkpointing
...
# -----------------------------------------------------------------------------
# Updated decode function (with <unk> for out-of-range tokens)
# -----------------------------------------------------------------------------
def decode(tokens):
...
class ShardedDataLoader:
    def __init__(self, data_dir, pattern, B, T, split="train"):
        """
        Loads tokens from bin files found in data_dir matching the pattern.
    ...
    def close_current_mmap(self):
        # Properly close memory-mapped file if it exists
        if self.full_data is not None:
            del self.full_data
            self.full_data = None

    def load_shard(self, filepath):
        self.close_current_mmap()  # close the previous shard if any
    ...
    def next_batch(self):
        ...
        # Additional check: clamp to ensure no invalid indices
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # Zero-init the projection weights
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GPT MoE Training with Rotary, RMSNorm, LR Warmup, Validation + Muon + Checkpointing"
    )
    ...
    # Add weight decay if desired for additional regularization
    optimizer_adam = torch.optim.AdamW(adam_params, lr=learning_rate, betas=(0.9, 0.95), weight_decay=0.01)
            ...
            with torch.profiler.profile(
                ...
                for i in tqdm(range(max_iters), desc="Training"):
                    ...
                    try:
                        ...
                        print("[Error] CUDA OOM. Skipping batch.")
                        continue
                    ...
                    if i % 10 == 0:
                        ...
                        msg = (f"[Training] Step {i}: Loss={loss.item():.4f}, "
                               f"ValLoss={v_loss.item():.4f}, "
                               f"IterTime={iter_time*1000:.2f} ms, "
                               f"Tokens={tokens_processed}, MFU={mfu:.2f}%")
                        print(msg)
